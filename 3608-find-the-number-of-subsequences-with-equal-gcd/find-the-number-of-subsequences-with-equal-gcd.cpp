...
public:
    int subsequencePairCount(vector<int>& nums) {
        ...
        // Now create a dp array to store every possible number of ways for every possible g1 and g2.
        ...
        for(int num: nums) {
            ...
            for(int g1 = 0; g1 <= max_val; g1++) {
                for(int g2 = 0; g2 <= max_val; g2++) {
                    ...
                    // 1. Either skip the current element
                    next_dp[g1][g2] = (next_dp[g1][g2] + ways)  % MOD;

                    // 2. Add current element to subsequence 1
                    int next_g1 = gcd(g1, num);
                    next_dp[next_g1][g2] = (next_dp[next_g1][g2] + ways) % MOD;

                    // 3. Add current element to subsequence 2
